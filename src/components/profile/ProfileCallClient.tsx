...
const buildRoute = (path: string, params: Record<string, string | undefined>) => {
...
export default function ProfileCallClient({
  initialName,
  initialRole,
  userId,
  uid,
  phone,
}: ProfileCallClientProps) {
  ...
  const chatRoute = useMemo(
    ...
    [name, phoneValue, role, uid, userId]
  );
    ...
    <div className="min-h-screen px-6 py-8 lg:px-10">
      ...
      <main className="mt-8 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-2xl border border-white/10 bg-[#2a2a2a] p-4">
          <p className="text-sm font-semibold text-white">Pronto para ligar</p>
          <p className="mt-2 text-xs text-[var(--text-muted)]">
            Use o botão abaixo para iniciar a ligação no seu dispositivo.
          </p>
          <div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-[var(--text-muted)]">
            <span className="rounded-full bg-[#1f1f1f] px-3 py-2">
              Número:{" "}
              <span className="text-white">{phoneValue ?? "Não informado"}</span>
            </span>
            {canCall ? (
              <a
                href={`tel:${sanitizedPhone}`}
                className="rounded-full bg-gradient-to-r from-[#c97200] to-[#fbb125] px-4 py-2 text-xs font-semibold text-black transition hover:brightness-110"
              >
                Iniciar ligação
              </a>
            ) : (
              <span className="rounded-full bg-[#1f1f1f] px-4 py-2 text-xs text-[var(--text-muted)]">
                Adicione um telefone para ligar
              </span>
            )}
          </div>
        </div>
        ...
        <aside className="space-y-4">
          <div className="rounded-2xl border border-white/10 bg-[#2a2a2a] p-4">
            ...
            <div className="mt-3 space-y-2 text-xs text-[var(--text-muted)]">
              <p>
                Contato: <span className="text-white">{name}</span>
              </p>
              <p>
                Função: <span className="text-white">{role}</span>
              </p>
              <p>Canal: Ligação direta</p>
            </div>
          </div>
          <div className="rounded-2xl border border-white/10 bg-[#2a2a2a] p-4 text-xs text-[var(--text-muted)]">
            <p className="text-white">Dica</p>
            <p className="mt-2">
              Você pode gravar o histórico de chamadas e status de atendimento
              com um provedor de voz, caso queira métricas completas.
            </p>
          </div>
        </aside>
      </main>
